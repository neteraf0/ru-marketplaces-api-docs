...
**operationId:** `DraftSupplyCreateStatus`
...
|-------|------|-----|-------------|
| `draft_id` | integer | ✓ | Идентификатор черновика. Получите значение параметра методом [/v2/draft/supply/create](#operation/DraftSupplyCreate). |
## Responses
...
| `status` | v2DraftSupplyCreateStatusResponseStatusEnum |  | Статус создания заявки на поставку:  - `UNSPECIFIED` — не определён,  - `SUCCESS` — создана,  - `IN_PROGRESS` — создаётся,  - `FAILED` — не удалось создать.  |

[Response 200](../_shared/examples/POST__v2_draft_supply_create_status_200.json)
