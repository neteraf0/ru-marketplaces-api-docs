...
**operationId:** `ReviewChangeStatusV2`
...
|-------|------|-----|-------------|
| `review_ids` | array |  | Список идентификаторов отзывов. |
| `status` | review.v2.ReviewChangeStatusV2Request.Status.Enum |  | Новый статус отзыва: - `NEW` — новый; - `VIEWED` — просмотренный; - `PROCESSED` — обработанный.  |

[Request example](examples/POST__v2_review_change_status_req.json)
