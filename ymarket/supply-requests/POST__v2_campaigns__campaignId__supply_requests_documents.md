...
|-------|------|-----|-------------|
| `requestId` | integer | ✓ | Идентификатор заявки.  {% note warning "Используется только в API" %}  По нему не получится найти заявки в кабинете продавца на Маркете. Для этого используйте `marketplaceRequestId` или `warehouseRequestId`.  {% endnote %}  |
## Responses
...
[Response 200](../_shared/examples/POST__v2_campaigns__campaignId__supply_requests_documents_200.json)
