...
| `imtId` | query | integer |  | Merged product cards ID. The same for all WB article of group of merged product cards. Every product card has `imtId`, even if is not merged with any other card  *Example: `256972151`* |
...
| `next` | query | integer |  | The last pinning operation ID (paginator)  *Example: `741`* |
| `limit` | query | integer |  | Feedback number per page (pagination)  *Example: `100`* |
...
[Response 200](../_shared/examples/GET__api_feedbacks_v1_pins_200.json)
