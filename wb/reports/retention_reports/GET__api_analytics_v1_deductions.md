...
|------|----|------|----------|-------------|
| `dateFrom` | query | string |  | Report period start. By default the date and time when data for the report was first received is used *Example: `2025-02-01T15:00:00Z`* |
| `dateTo` | query | string | ✓ | Report period end *Example: `2025-10-11T18:00:00Z`* |
| `sort` | query | string (enum: nmId, dtBonus, bonusSumm) |  | Sorting: - `nmId` — by Wildberries article - `dtBonus` — by fine date and time - `bonusSumm` — by fine amount  *Example: `nmId`* |
| `order` | query | string (enum: desc, asc) |  | Data order: - `desc` — descending - `asc` — ascending  *Example: `asc`* |
