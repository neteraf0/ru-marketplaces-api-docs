...
The method transfers [assembly orders](./orders-dbs#tag/DBS-Assembly-Orders) with the `new` and `confirm` [statuses](./orders-dbs#tag/DBS-Assembly-Orders/paths/~1api~1marketplace~1v3~1dbs~1orders~1status~1info/post) to the `cancel` status  — canceled by the supplier.
You can not cancel assembly orders with the `deliver` status.
